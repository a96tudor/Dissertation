...
\begin{document}
	\begin{tikzpicture}
		...
		\draw[dotted, thick] (1,7) -- (3,6.5);  
		\draw[dotted, thick] (1,4) -- (3,3.5);  
		
		\draw[dotted, thick] (6,6) -- (4,6.5);  
		\draw[dotted, thick] (6,5) -- (4,3.5);  
		
		\node () at (0.5,10) {\textbf{Input}};
		\node () at (3.5,10) {\textbf{Kernel}};
		\node () at (6.5,10) {\textbf{Output}};
		
		\node () at (2, 5) {\LARGE$\mathbf{*}$};
		\node () at (5, 5) {\LARGE$\mathbf{=}$};
		
		\draw[fill=green!10] (0,4) rectangle (1,7);
		...
		\draw[fill=red!10] (6,5) rectangle (7,6); 	
	
		\foreach \m/\l [count=\y] in {miss,miss,miss,3,1,1}
		{
